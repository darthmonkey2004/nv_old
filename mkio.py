...
#The data stored by this module can be imported with the following python commands (assuming file name is capture1_out.py):
#	import capture1_out as iodata
#	data = iodata.showlist()
#	print (data)
def mkIOFile(list_object, iofile_path = "capture1_out"):#define function to export list or tuple
	try:#enclosed in a try/except block just in case the iterable test fails it doesn't throw an exception and kill your code
		...
		#This code block below is where the magic happens. It writes a function into a text file(name of file stored in iofile_path) containing the list you passed to this function earlier.
		outstring = ("def showlist():" + '\n' + "	newlist = " + str(list_object) + '\n' + "	return (newlist)" + '\n')
		fname = (iofile_path + ".py")# set the file name to a .py extension to enable python's import function.
